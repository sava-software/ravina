...
prints the operator rules reproduced verbatim below. Both are registered per
...
Generated by the installed plugin and reproduced verbatim between the
`block:start` / `block:end` markers below, with one digest marker on the line
after `block:end`. Those markers bound the shared template and nothing else:
every Ravina-specific ownership fact, measurement, provenance note, acceptance
reason and gotcha lives outside them, and nothing inside them is hand-edited.

Re-sync only through the project-qualified pair
`./gradlew :ravina-core:hardeningAgentTemplate` (prints the template this plugin
version carries, with its markers and digest) and
`./gradlew :ravina-core:hardeningAgentTemplateDiff` (read-only; diffs the bounded
block against it). Run the diff on every template-digest move, and replace the
acknowledged digest rather than adding one. `agentsTemplateInSync` gates that
acknowledgment from `check` and `qualityGate`; when it fails versus warns is the
installed task's business, not this file's — read what it prints.
