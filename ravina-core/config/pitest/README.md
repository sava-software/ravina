...
1. **Kill it** — add or strengthen a test. Prefer asserting the property the
   mutant breaks (pacing as an exact function of requested delays, capacity
   after a dock, order after a sort) over restating the implementation.
...
**Unobservable timers** — call-time measurement mutants
(`System.nanoTime`/duration bookkeeping) on paths where `measureCallTime` is
false: the measured value is never read.
